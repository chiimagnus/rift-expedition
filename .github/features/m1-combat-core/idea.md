...
6. **可达域计算**（对应 [A/06] 地形×移动 + [E/20] 寻路）：给定起点、移动力、地形网格，用 Dijkstra（按地形 moveCost 加权、按兵种类型 foot/horse/fly 分别计费）算出可达格集合；`moveCost=∞` 视为不可通行（骑兵不可进森林以上密度地形）。本阶段只需可达域判定，不需要路径重建/寻路可视化（那是 M2 需要"走位动画"时才加，A\* 路径重建延后）。**地形网格用地形 id 二维数组表示**（`string[][]`，`grid[y][x]`，行优先），不是完整 `TerrainDef[][]` 对象数组——函数内部自行查 `TERRAIN_BY_ID` 解析数值。这样做是为了让下面第 9 条 `BattleState.grid` 可以直接复用同一种「id 网格」表示，存档序列化时不需要额外做对象↔id 的转换，也不会把整份地形静态目录数据重复内嵌进每一份运行时状态/存档里。
...
9. **运行时战斗状态模型 `BattleState`/`UnitInstance`（新增，供 M1 Phase 2 结算服务与 M2 全部 phase 共用）**：审计 M2 垂直切片规划时发现，M2 的 Phase 1（移动+渲染）与后续 phase（攻击/存档）都需要一个「运行时战斗实例」类型——区别于本 feature 已有的 3 张**静态**目录（`TERRAIN_DEFS`/`WEAPON_DEFS`/`UNIT_DEFS`），但这个类型此前未在 M1 或 M2 任何一份文档里被真正定义过（只在 M2 文档里被当作「来自 M1 models」引用，实际并不存在）。若把它放进 M2 定义，会让**尚未规划的 M1 Phase 2（战斗结算）反过来依赖 M2 的交付物**，破坏 [F/24] 规定的 `M0 → M1 → M2` 单向依赖顺序。因此本条决定：该类型属于 M1 Phase 1 的交付物（放 `src/models/battleState.ts`，models 层，纯数据无行为，符合 M1 分层边界），M1 Phase 2 结算服务与 M2 各 phase 都直接复用，不得各自重新发明。最小字段集：
   ```typescript
   ...
   export interface UnitInstance {
     ...
     weaponId?: string       // 指向 WEAPON_DEFS[i].id，M1 Phase1 可选，Phase2 结算需要
     faction: Faction
   ...
   export interface BattleState {
     gridWidth: number
     gridHeight: number
     grid: string[][]        // grid[y][x] = 地形 id（对应 TERRAIN_BY_ID 的 key），行优先
     units: UnitInstance[]
     phase: BattlePhase       // 4 态一次性定义完整（含 M2 才用到的 'deploy'），Phase1 只使用其中部分值不算违反非目标
     rngState: number         // 与 src/services/prng.ts 的 PrngState 同形状，但本文件禁止 import 该服务类型（见下）
     turnCount: number
   }
   ```
   **重要分层约束**：`src/models/battleState.ts` 严禁 `import` `src/services/prng.ts`（即使只是 `import type`）——models 层不得反向依赖 services 层（见「移交备注 6」）。`rngState` 直接声明为裸 `number`，与 `PrngState`（`src/services/prng.ts` 里的类型别名，本质也是 `number`）保持数值兼容即可，不需要类型层面的显式引用。
...
- 不实现存档系统（localStorage/SaveGame/版本迁移）——见 [E/21]，M1 是纯内存战斗模拟，无持久化需求；`BattleState`（核心需求 9）只是内存态形状定义，不含任何持久化读写代码。
...
- [ ] `src/models/battleState.ts` 导出的 `BattleState`/`UnitInstance` 类型完整（字段齐全），且该文件不 `import` `src/services/*` 中任何模块（单测/静态检查双重确认分层不违反）。
...
6. 分层依赖方向不能反：`src/services/*` 可以 `import` `src/models/*`（通过 `@models/*` 别名），但 `src/models/*` 不能 `import` `src/services/*` 之类的上层——这一条对新增的 `src/models/battleState.ts` 尤其重要：它的 `rngState` 字段类型是裸 `number`，禁止为了"看起来更规范"而 `import type { PrngState } from '@services/prng'`。
