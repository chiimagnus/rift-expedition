...
- 地图编辑源是 Tiled（`.tmx`），运行时由 [SKTiled](https://github.com/mfessenden/SKTiled) 读取渲染
...
项目已提交生成好的 `RiftExpedition.xcodeproj`，可直接用 Xcode 打开运行。

如果改动了 `project.yml`（新增依赖、target 或资源目录），需要用 [XcodeGen](https://github.com/yonaskolb/XcodeGen) 重新生成工程文件：
