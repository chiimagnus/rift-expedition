...
Non-goals：不写伤害/命中/追击公式（Phase 2）；不写平衡 CLI/贪心 AI/BattleState 状态机（Phase 3）；不碰 UI/viewmodels/entrypoints。
...
## P1-T7 更新根 AGENTS.md 记录 M1 Phase1 新增模块
...
提交：`git add AGENTS.md && git commit -m "docs: P1-T7 - 记录 M1 Phase1 新增模块"`。

---

## Phase 1 完成后的审计约定（audit-p1.md）

Phase 1 全部 7 个 task 完成并提交后，必须进入 `audit-p1.md` 审计闭环，重点检查：
